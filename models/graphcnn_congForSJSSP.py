...
class GraphCNN(nn.Module):
    ...
    def next_layer(self, h, layer, padded_neighbor_list=None, Adj_block=None):
        ...
        if self.neighbor_pooling_type == "max":
            ...
            # print(h.dtype)
            pooled = torch.mm(Adj_block, h)  # 邻接矩阵与特征矩阵的卷积操作
            if self.neighbor_pooling_type == "average":
                # If average pooling 计算每个节点的邻居节点的数量
                degree = torch.mm(Adj_block, torch.ones((Adj_block.shape[0], 1)).to(self.device))
                pooled = pooled/degree
        # representation of neighboring and center nodes

        pooled_rep = self.mlps[layer](pooled)  # 进行多层感知机操作
        h = self.batch_norms[layer](pooled_rep)  # 批量归一化
