...
def min_mch_job(mch_time, mchsEndTimes, number_of_machines, dur, temp, first_col):
    ...
    while True:
        ...
        # dur(job,op,machine)->(machine,number_of_operations)
        min_mchForJob = []
        dur = np.copy(dur)
    ...
    for z in range(len(min_mch_time)):
        ...
        job_action_space = min[np.where(time <= mch_time.min())] if len(np.where(time <= mch_time.min())[0]) != 0 else \
        min[np.argmin(time)]
...
def min_job_mch(CT_mch, CT_job, mchsEndTimes, number_of_machines, dur, CT_ope, ope_ids, mask_job, done, mask_ope_mch):
    """
    ...
    :param done:
    :param mask_ope_mch:
    :return: 加工时间最小的机器编号, 最早开始加工的工序, 工件蒙版, 工序蒙版
    """
    CT_ope = np.copy(CT_ope)
    for machine, j in zip(mchsEndTimes, range(number_of_machines)):
        if np.all(machine == -configs.high):
            CT_mch[j] = 0
        else:
            CT_mch[j] = machine[np.where(machine >= 0)][-1]
    for job, j in zip(CT_ope, range(CT_ope.shape[0])):
        if np.all(job == 0):
            CT_job[j] = 0
        else:
            CT_job[j] = job[np.where(job != 0)][-1]
    CT_job1 = np.copy(CT_job)
    '''if np.all(job_time) != 0:
        job_time = np.delete(job_time, np.where(job_time == job_time.min())[0])'''
    while True:
        mask_job1 = np.full(shape=(CT_ope.shape[0]), fill_value=1, dtype=bool)  # 初始化
        min_ct_job_idx = np.where(CT_job1 <= CT_job.min())[0]  # 完工时间最小的工件序号集合
        min_ct_ope_idxs = ope_ids[min_ct_job_idx]
        dur = deepcopy(dur)
        dur = dur.reshape(-1, number_of_machines)
        mch_minOpe = []
        for ope in min_ct_ope_idxs:
            mch_for_job = np.where(dur[ope] > 0)[0]
            mch_minOpe.append(mch_for_job)
        minMch_For_minOpe = []
        mask_ope_mch1 = np.copy(mask_ope_mch)
        '''for i in range(len(min_task)):
            ...
            if len(np.where(CT_mch[mchtask] <= job_time.min())[0]) != 0:
                mch_action_space = mchtask[np.where(CT_mch[mchtask] <= job_time.min())].tolist()
        ...
        for i in min_ct_ope_idxs:  # 这里相当于选工件的时候设置了一个调度规则，选择加工时间最短的工件
            mask_job1[np.where(ope_ids == i)] = 0  # True：遮住，不能选，False：没遮住，可以选
        mask_job1 = mask_job + mask_job1
        if done:
            break
        elif np.all(mask_job1) == True:
            CT_job = np.delete(CT_job, np.where(CT_job == CT_job.min())[0])
        else:
            break
    mch_space = minMch_For_minOpe
    return mch_space, min_ct_ope_idxs, mask_job1, mask_ope_mch1
